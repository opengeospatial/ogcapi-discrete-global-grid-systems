...
edge(s) between those two points is within the contour. A DGGS-FG-JSON representation is requested using the `application/fg+json` media type together with the `geometry=quantized` (or `geometry=quantized-zoneids`) query parameter.
