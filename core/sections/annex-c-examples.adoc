...
This first example retrieves elevation data for the GEBCO collection for ISEA3H zone `A2-0-A` at a relative depth of 8 (specified using the `zone-depth` parameter -- the
default zone depth is 10 for this DGGRS in this particular deployment of the API, as specified in the
`defaultDepth` property of the <<isea3h-dggs-description-response,ISEA3H DGGRS description response>>).
...
If the geometry for these zone features is provided as polygons, visualizing this vector data would produce an output like the following partial response,
...
Alternatively, the geometry of each zone could also be provided as a single point for its centroid, in which case the response and associated visualization would look like the following:
...
In DGGS-FG-JSON, spatial coordinates of the feature geometries croresponding to the spatial dimensions of the DGGRS are encoded as integer sub-zone order indices ranging from 1 to the
...
edge(s) between those two points is within the contour.
...
For the same reason, a https://docs.ogc.org/is/17-083r4/17-083r4.html[2D Tile Matrix Set] can be defined in this 5x6 space, which supports using
...
Using centroid points for the geometry of each zone, the GeoJSON response and associated visualization would look like the following:
...
With the artificial edge information of DGGS-FG-JSON, a client could avoid such visualization artifacts, and could more easily recombine geometry split across separate tiles.
...
https://maps.gnosis.earth/ogcapi/collections/climate:cmip5:singlePressure/dggs/ISEA3H/zones/A1-0-E/data?zone-depth=8&datetime=2022-10-06/2022-10-10
...
https://maps.gnosis.earth/ogcapi/collections/climate:era5:relativeHumidity/dggs/ISEA3H/zones/A4-0-A/data?zone-depth=8&datetime=2023-04-06T23:00:00&subset=pressure(750:900)
...
https://maps.gnosis.earth/ogcapi/collections/climate:era5:relativeHumidity/dggs/ISEA3H/zones/A4-0-A/data?zone-depth=8&datetime=2023-04-06T23:00:00&subset=pressure(900)
