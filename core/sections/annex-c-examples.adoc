...
A snippet of the DGGS-JSON response follows, including an array of elevation values ordered as per the <<img_isea3h_order_vertex,deterministic zone order defined by this ISEA3H DGGRS>>.
...
resulting in a large separation between the two half of the data, as illustrated for example with zone C2-4-A.
...
image::images/isea9r-gebco-orthographic.png[width=700,align="center"]
...
a GeoTIFF representation provides a one-to-one mapping of zones to rectilinear gridded coverage cells, as can be seen by comparing this previous visualization the GeoTIFF response with the visualization of the DGGS-JSON responses.
...
TIP: As can be seen by comparing this previous visualization the GeoTIFF response with the visualization of the DGGS-JSON responses,
...
.Visualization of different SCL (Scene Classification Layer) for ISEA3H zone https://maps.gnosis.earth/ogcapi/collections/sentinel2-l2a/dggs/ISEA3H/zones/G7-66F79-F[G7-66F79-F] (K'gari, Australia) from https://sentinel.esa.int/web/sentinel/missions/sentinel-2[Copernicus SENTINEL-2 operated by ESA]
image::images/field-selection-scl.png[width=700,align="center"]

[keep-with-next=true]
The following request retrieves only the near-infrared (`B08`) for the same zone.
...
.Visualization of different B08 (near-infrared) band for ISEA3H zone https://maps.gnosis.earth/ogcapi/collections/sentinel2-l2a/dggs/ISEA3H/zones/G7-66F79-F[G7-66F79-F] (K'gari, Australia) from https://sentinel.esa.int/web/sentinel/missions/sentinel-2[Copernicus SENTINEL-2 operated by ESA]
