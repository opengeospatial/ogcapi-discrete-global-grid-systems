...
<<rc_data-ubjson,DGGS-UBJSON Data>> (DGGS-JSON encoded as https://ubjson.org[UBJSON]) +
 +
*For either DGGS-quantized raster data or traditional coordinate reference system (based on `profile` query parameter):* +
...
<<rc_data-dggs-ubjsonfg,DGGS-UBJSON-FG Data>> (DGGS-JSON-FG encoded as https://ubjson.org[UBJSON]) +
 +
*2D raster formats for interoperability:* +
...
<<rc_data-png,PNG Data>> (https://www.w3.org/TR/png/[PNG]) |
 +
*Vector format with traditional coordinate reference systems for interoperability:* +
...
(see <<_requesting_data_at_a_particular_relative_depth, Examples C.9>>) +
 +
*Defined in <<rc_data-dggs-jsonfg,DGGS-JSON-FG Data>>, <<rc_data-dggs-ubjsonfg,DGGS-UBJSON-FG Data>>, <<rc_data-geojson,GeoJSON Data>>, <<rc_data-netcdf,netCDF Data>>, <<rc_data-zarr,Zarr Data>>, <<rc_data-coveragejson,CoverageJSON Data>>:* (recommendations, optional for server to support) +
