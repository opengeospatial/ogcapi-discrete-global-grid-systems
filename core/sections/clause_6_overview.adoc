...
(see <<_requesting_data_at_a_particular_relative_depth, Examples C.9>>) +
 +
*Defined in <<rc_data-dggs-jsonfg,DGGS-JSON-FG Data>>, <<rc_data-dggs-ubjsonfg,DGGS-UBJSON-FG Data>> and <<rc_data-geojson,GeoJSON Data>>:* (recommendations, optional for server to support) +
