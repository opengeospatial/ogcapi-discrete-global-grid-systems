...
* The Zonal Identifier Reference System (`zirs`), with textual identifiers being mandatory while 64-bit integer identifiers (which support the <<rc_zone-binary64bit,64-bit integer zone list encoding requirements class>>) are optional,
...
based on either the ASCII character strings of the textural identifiers, or the 64-bit integer identifiers.
