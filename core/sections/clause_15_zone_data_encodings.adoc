...
However, it does define requirements classes for zone data encodings which are expected to be commonly supported in implementations of this standard.
...
|DGGS-FG-JSON zone data   |application/fg+json (with `geometry=quantized` query parameter)
|DGGS-FG-UBJSON zone data |application/fg+ubjson (with `geometry=quantized` query parameter)
