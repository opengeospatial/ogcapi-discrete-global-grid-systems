...
part:: The Implementation SHOULD include in the response a `returnedArea` property (in the case of a 2D DGGS) or `returnedVolume` property (in the case of a 3D or 4D DGGS) indicating the total surface area (or total volume) occupied by the zones being returned,
