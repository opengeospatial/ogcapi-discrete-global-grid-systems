...
a `zoneID` property of that feature corresponds to the textual identifier (the `{zoneId}`), the `id` of the feature corresponds to either a sequential feature identifier starting at 1, the textural identifier of the zone, or the 64-bit unsigned integer identifier of the zone,
