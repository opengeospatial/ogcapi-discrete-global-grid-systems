...
3+|^1^ The _DGGRS List_ (`.../dggs`), _DGGRS Description_ (`.../dggs/{dggsId}`) and _DGGRS Zone Information_ (`.../dggs/{dggsId}/zones/{zoneId}`) resources are defined in requirements class "Core".
3+|^2^ The _DGGRS Zones_ resource (`.../dggs/{dggsId}/zones`) is defined in the "Zone Query" requirements class.
3+|^3^ The _DGGRS Zone Data_ resource (`.../dggs/{dggsId}/zones/{zoneId}/data`) is defined in the "Data Retrieval" requirements class.
